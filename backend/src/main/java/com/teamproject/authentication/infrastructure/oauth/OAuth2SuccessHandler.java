...
public class OAuth2SuccessHandler implements AuthenticationSuccessHandler {
    ...
    public OAuth2SuccessHandler(OAuthLoginService oauthLogin, RefreshCookieService cookies, @Value("${app.frontend-url}") String frontendUrl) {
        this.oauthLogin = oauthLogin; this.cookies = cookies; this.frontendUrl = frontendUrl;
    }
    @Override public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication)
        ...
        try {
            var tokens = oauthLogin.login(provider, profile.subject(), profile.email(), profile.name());
