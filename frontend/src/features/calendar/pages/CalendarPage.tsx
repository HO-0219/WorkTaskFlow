...
const typeLabels: Record<CalendarItem['type'], [string, string]> = {
  SCHEDULE: ['일정', 'Event'], MEETING: ['회의', 'Meeting'], VACATION: ['휴가', 'Vacation'], TODO: ['할 일', 'To do'], DEADLINE: ['업무 기간', 'Task timeline'],
};
...
export function CalendarPage() {
  ...
  const filteredItems = items.filter((item) => {
    ...
    const ownerMemberId = item.source === 'TASK_DEADLINE' ? item.taskAssigneeMemberId : item.ownerMemberId;
    if (ownerFilter === 'unassigned') return item.source === 'TASK_DEADLINE' && !item.taskAssigneeMemberId;
    if (ownerFilter === 'me') return groups.some((group) => group.id === item.groupId && group.memberId === ownerMemberId);
    const [filterGroupId, memberId] = ownerFilter.split(':').map(Number);
    return item.groupId === filterGroupId && ownerMemberId === memberId;
  ...
  const selectedItems = filteredItems.filter((item) => itemOccursOnDate(item, selectedDate));
  return <><AppNavigation /><main className="calendar-page app-page">
    <header className="calendar-header"><div><div><span className="page-eyebrow">SCHEDULE</span><h1>{t('캘린더', 'Calendar')}</h1><p>{t('팀 일정과 업무 시작·진행·마감 흐름을 한눈에 살펴보세요.', 'See team events and task timelines from start through due date.')}</p></div></div><div className="calendar-header-actions"><button className="secondary" type="button" onClick={() => setModal('import')}>⇩ {t('일정 가져오기', 'Import')}</button><button className="primary create-action" type="button" onClick={() => openCreate()}><span>＋</span> {t('일정 추가', 'Add event')}</button></div></header>
    ...
    <section className="calendar-shell">
      ...
      <div className="month-calendar" aria-label={t(`${month.getFullYear()}년 ${month.getMonth() + 1}월 달력`, `${formatMonth(month, language)} calendar`)}>
        ...
        {days.map((day) => {
          const date = dateText(day); const dayItems = filteredItems.filter((item) => itemOccursOnDate(item, date));
          ...
          return <div className={`calendar-day ${current ? '' : 'outside'} ${isToday ? 'today' : ''} ${selected ? 'selected' : ''} ${draggingId ? 'drop-ready' : ''}`} key={date}
            ...
            <div className="day-events">{dayItems.slice(0, 3).map((item) => item.source === 'TASK_DEADLINE'
              ? <button className={`day-event task-timeline ${taskTimelineClass(item)} ${taskRangeClass(item, date, day.getDay())}`} type="button" onClick={(event) => { event.stopPropagation(); select(item); }} key={`task-${item.sourceTaskId}`} aria-label={`${item.title} · ${taskTimelineLabel(item, language)}`} title={`${item.title} · ${taskTimelineLabel(item, language)}`}><span aria-hidden="true">{date === item.startAt.slice(0, 10) ? '▶' : date === item.endAt.slice(0, 10) ? '◆' : ''}</span>{showsTaskTimelineTitle(item, date, day.getDay()) ? item.title : ''}</button>
      ...
      <div className="calendar-legend"><span><i className="legend-schedule" />{t('일정', 'Event')}</span><span><i className="legend-unassigned" />{t('업무·미지정', 'Task · unassigned')}</span><span><i className="legend-assigned" />{t('담당 지정', 'Assigned')}</span><span><i className="legend-progress" />{t('진행 중', 'In progress')}</span><span><i className="legend-hold" />{t('보류', 'On hold')}</span><span><i className="legend-delayed" />{t('지연', 'Overdue')}</span><span><i className="legend-completed" />{t('완료', 'Completed')}</span></div>
    </section>
    <section className="selected-day-panel"><header><div><span>{formatSelectedDate(selectedDate, language)}</span><h2>{t('이날의 업무와 일정', 'Tasks and events')}</h2></div><button className="secondary" type="button" onClick={() => openCreate(new Date(`${selectedDate}T00:00:00`))}>＋ {t('일정 추가', 'Add event')}</button></header>
      {selectedItems.length === 0 ? <p className="empty-state">{t('등록된 업무나 일정이 없습니다.', 'No tasks or events on this day.')}</p> : <div className="selected-day-list">{selectedItems.map((item) => <button type="button" onClick={() => select(item)} key={`${item.source}-${item.eventId ?? item.sourceTaskId}`}><i className={item.source === 'TASK_DEADLINE' ? taskTimelineClass(item) : `type-${item.type.toLowerCase()}`} /><span><strong>{item.title}</strong><small>{item.allDay ? t('종일', 'All day') : item.type === 'DEADLINE' ? `${item.startAt.slice(0, 10)} → ${item.endAt.slice(0, 10)} · ${taskTimelineLabel(item, language)}` : `${item.startAt.slice(11, 16)} – ${item.endAt.slice(11, 16)}`} · {item.groupName}</small></span><b>›</b></button>)}</div>}
    </section>
...
function formatItemPeriod(item: CalendarItem, language: 'ko' | 'en') {
  ...
  if (item.type === 'DEADLINE') {
    const end = new Intl.DateTimeFormat(language === 'ko' ? 'ko-KR' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'short' }).format(new Date(`${item.endAt.slice(0, 10)}T00:00:00`));
    return `${date} ${item.startAt.slice(11, 16)} → ${end} ${item.endAt.slice(11, 16)}`;
  }
...
function itemOccursOnDate(item: CalendarItem, date: string) { return item.source === 'TASK_DEADLINE'
  ? item.startAt.slice(0, 10) <= date && item.endAt.slice(0, 10) >= date
  : item.startAt.slice(0, 10) === date; }
function taskTimelineClass(item: CalendarItem) { if (item.taskDelayed) return 'timeline-delayed'; if (!item.taskAssigneeMemberId) return 'timeline-unassigned'; return ({ IN_PROGRESS: 'timeline-progress', ON_HOLD: 'timeline-hold', COMPLETED: 'timeline-completed', REJECTED: 'timeline-closed', CANCELLED: 'timeline-closed' } as Record<string, string>)[item.taskStatus ?? ''] ?? 'timeline-assigned'; }
function taskTimelineLabel(item: CalendarItem, language: 'ko' | 'en') { if (item.taskDelayed) return language === 'ko' ? '지연' : 'Overdue'; if (!item.taskAssigneeMemberId) return language === 'ko' ? '미지정' : 'Unassigned'; const labels: Record<string, [string, string]> = { REQUESTED: ['승인 대기', 'Pending approval'], TODO: ['담당 지정', 'Assigned'], IN_PROGRESS: ['진행 중', 'In progress'], ON_HOLD: ['보류', 'On hold'], COMPLETED: ['완료', 'Completed'], REJECTED: ['반려', 'Rejected'], CANCELLED: ['취소', 'Cancelled'] }; return (labels[item.taskStatus ?? ''] ?? ['업무', 'Task'])[language === 'ko' ? 0 : 1]; }
function taskRangeClass(item: CalendarItem, date: string, weekday: number) { const starts = date === item.startAt.slice(0, 10) || weekday === 0; const ends = date === item.endAt.slice(0, 10) || weekday === 6; return starts && ends ? 'range-single' : starts ? 'range-start' : ends ? 'range-end' : 'range-middle'; }
function showsTaskTimelineTitle(item: CalendarItem, date: string, weekday: number) { return date === item.startAt.slice(0, 10) || weekday === 0; }
