...
export function CalendarPage() {
  ...
  return <><AppNavigation /><main className="calendar-page app-page">
    ...
    <section className="calendar-shell">
      <div className="calendar-toolbar"><div className="month-controls"><button type="button" aria-label={t('이전 달', 'Previous month')} onClick={() => setMonth(addMonth(month, -1))}>‹</button><strong aria-live="polite">{formatMonth(month, language)}</strong><button type="button" aria-label={t('다음 달', 'Next month')} onClick={() => setMonth(addMonth(month, 1))}>›</button><button className="today-button" type="button" onClick={() => setMonth(new Date(today.getFullYear(), today.getMonth(), 1))}>{t('오늘', 'Today')}</button></div>
