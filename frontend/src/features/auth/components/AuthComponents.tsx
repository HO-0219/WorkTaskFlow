...
export function AuthLayout({ title, description, children }: {
  ...
  const { t } = useLanguage();
  return <main className="auth-page"><section className="brand"><span className="brand-mark"><BrandMark /></span><p>{t('퇴근', 'TOEGEUN')}</p><h1>{t('업무는 남기고,', 'Keep the work,')}<br />{t('야근은 남기지 마세요.', 'not the overtime.')}</h1><span>{t('요청부터 완료와 리포트까지. 퇴근을 사수하는 팀 업무관리.', 'From requests to reports—a team workspace built to help work finish on time.')}</span></section><section className="auth-card"><header><Link to="/" className="mobile-logo"><BrandMark />{t('퇴근', 'toegeun')}</Link><h2>{title}</h2><p>{description}</p></header>{children}</section></main>;
}
