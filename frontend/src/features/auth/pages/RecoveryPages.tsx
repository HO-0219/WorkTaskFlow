...
export function ResetPasswordPage() {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); if (password !== confirm) return setError('비밀번호가 서로 다릅니다.'); setPending(true); setError(''); try { await authApi.resetPassword(email, token, password); navigate('/login?reset=success'); } catch (e) { setError(errorMessage(e)); } finally { setPending(false); } }
