...
export function ProfilePage() {
  ...
  return <><AppNavigation /><main className="profile-page app-page"><header className="profile-page-header"><span className="page-eyebrow">MY PROFILE</span><h1>{t('프로필', 'Profile')}</h1><p>{t('나를 표현하는 정보를 편하게 관리하세요.', 'Manage how you appear to your teammates.')}</p></header><section className="profile-card-new">
    ...
    <form className="form" onSubmit={submit}>
      ...
      <label className="field"><span>{t('프로필 이미지', 'Profile image')}</span><input type="file" accept="image/jpeg,image/png,image/gif" disabled={uploading} onChange={event => uploadImage(event.target.files?.[0])} /><small className="field-help">{uploading ? t('업로드 중...', 'Uploading...') : t('JPG, PNG, GIF · 최대 5MB', 'JPG, PNG, GIF · up to 5MB')}</small></label>
