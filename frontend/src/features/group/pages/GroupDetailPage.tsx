...
export function GroupDetailPage() {
  ...
  return <><AppNavigation /><main className="group-detail-page app-page"><section className="auth-card profile-card group-detail-card">
    ...
    {group?.type === 'TEAM' && settingsTab === 'collaboration' && <section className="member-directory-link-card"><div><span className="page-eyebrow">RESOURCES</span><h2>{t('그룹 자료', 'Group resources')}</h2><p>{t('기본 자료함과 프로젝트 파일을 파일 화면에서 한 번에 관리하세요.', 'Manage the default folder and every project’s files from the Files screen.')}</p></div><Link className="primary" to={`/groups/${group.id}/files`}>{t('파일 화면 열기', 'Open Files')} →</Link></section>}
