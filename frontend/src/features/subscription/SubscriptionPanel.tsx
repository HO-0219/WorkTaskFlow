...
export function SubscriptionPanel({ groupId }: { groupId: number }) {
  ...
  useEffect(() => {
    Promise.all([subscriptionApi.get(groupId), reportApi.schedule(groupId), paymentApi.methods()])
      .then(([value, report, paymentMethods]) => {
        setSubscription(value); setSchedule(report);
  ...
  async function saveSchedule(event: FormEvent) {
    event.preventDefault(); if (!schedule) return; setPending(true); setError('');
    try {
      setSchedule(await reportApi.updateSchedule(groupId, {
        recipientEmail: schedule.recipientEmail, weeklyEnabled: schedule.weeklyEnabled,
  ...
  return <section className="subscription-panel group-subsection"><header className="group-section-heading"><div><span className="page-eyebrow">SUBSCRIPTION</span><h2>{t('구독과 자동 리포트', 'Subscription & scheduled reports')}</h2><p>{t('무료 체험과 운영 전환 선택, 주간·월간 리포트 메일을 관리합니다.', 'Manage trials, conversion, and weekly/monthly report emails.')}</p></div><span className={`membership-badge ${subscription.status.toLowerCase()}`}>{subscriptionStatus(subscription.status, language)}</span></header>
    ...
    <form className="report-schedule-form" onSubmit={saveSchedule}><h3>{t('메일 리포트 일정', 'Email report schedule')}</h3>
      <label><input type="checkbox" checked={schedule.weeklyEnabled} disabled={!schedule.weeklyEligible} onChange={(event) => setSchedule({ ...schedule, weeklyEnabled: event.target.checked })} /> {t('주간 리포트', 'Weekly report')}</label>
      <select value={schedule.weeklyDay ?? 'MONDAY'} disabled={!schedule.weeklyEnabled} onChange={(event) => setSchedule({ ...schedule, weeklyDay: event.target.value })}>{weekdays.map(([value, ko, en]) => <option value={value} key={value}>{t(ko, en)}</option>)}</select>
      ...
      <label><input type="checkbox" checked={schedule.monthlyEnabled} disabled={!schedule.monthlyEligible} onChange={(event) => setSchedule({ ...schedule, monthlyEnabled: event.target.checked })} /> {t('월간 리포트', 'Monthly report')}</label>
      <select value={schedule.monthlyDay ?? 1} disabled={!schedule.monthlyEnabled} onChange={(event) => setSchedule({ ...schedule, monthlyDay: Number(event.target.value) })}>{Array.from({ length: 28 }, (_, index) => index + 1).map((day) => <option value={day} key={day}>{t(`${day}일`, `Day ${day}`)}</option>)}</select>
      ...
      <label>{t('언어', 'Language')}<select value={schedule.language} onChange={(event) => setSchedule({ ...schedule, language: event.target.value as ReportSchedule['language'] })}><option value="KO">한국어</option><option value="EN">English</option><option value="BOTH">{t('한글+영문', 'Korean + English')}</option></select></label>
      <label>{t('수신 이메일', 'Recipient email')}<input type="email" value={schedule.recipientEmail} readOnly /></label>
      <button className="primary" disabled={pending}>{pending ? t('저장 중...', 'Saving...') : t('메일 일정 저장', 'Save schedule')}</button>
