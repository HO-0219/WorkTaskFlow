...
export function GroupDashboardPage() {
  ...
  const [groupMembers, setGroupMembers] = useState<MemberResponse[]>([]);
  ...
  useEffect(() => {
    Promise.all([groupApi.get(groupId), groupApi.members(groupId), projectApi.list(groupId), taskApi.list(groupId), emergencyIssueApi.list(groupId)]).then(async ([value, members, projectValues, taskValues, urgentValues]) => {
      const activeProjects = projectValues.filter((project) => project.status !== 'ARCHIVED');
      setGroup(value); setGroupMembers(members.filter((member) => member.status === 'ACTIVE'));
      setProjects(activeProjects); setAllTasks(taskValues);
      setEmergencyIssues(urgentValues);
      const topicEntries = await Promise.all(activeProjects.map(async (project) => {
        try { return [project.id, (await projectIssueApi.list(project.id)).filter((topic) => topic.level === 'MAJOR' && !topic.archivedAt)] as const; }
        catch { return [project.id, []] as const; }
      }));
      setTopicsByProject(Object.fromEntries(topicEntries));
      setReportScope(value.role === 'LEADER' ? 'GROUP' : 'MY');
    }).catch((value) => setError(errorMessage(value)));
  }, [groupId]);
  useEffect(() => {
    if (!projectId) { setTopics([]); setTopicId(''); return; }
    projectIssueApi.list(Number(projectId)).then((values) => setTopics(values.filter((value) => value.level === 'MAJOR' && !value.archivedAt)))
  ...
  useEffect(() => subscribeToLiveUpdates((update) => {
    if (update.groupId === groupId) { void load(true); taskApi.list(groupId).then(setAllTasks).catch(() => undefined); }
  ...
  async function createTask(event: FormEvent) {
    ...
    try {
      ...
      await taskApi.create(groupId, { title: title.trim(), description: description.trim() || undefined, priority, dueAt: dueAt || undefined, projectId: projectId ? Number(projectId) : undefined, projectTopicId: topicId ? Number(topicId) : undefined, checklistItems: checklist.length > 0 ? checklist : undefined });
      setTitle(''); setDescription(''); setPriority('NORMAL'); setDueAt(''); setProjectId(''); setTopicId(''); setChecklistItems([]); setShowCreate(false);
      ...
      setAllTasks(await taskApi.list(groupId));
  ...
  const activeTeamTasks = allTasks.filter((task) => !['COMPLETED', 'REJECTED', 'CANCELLED'].includes(task.status));
  const unlinkedTasks = activeTeamTasks.filter((task) => task.projectId == null);
  const openEmergencyIssues = emergencyIssues.filter((issue) => issue.status === 'OPEN');
  return <><AppNavigation /><main className="group-dashboard-page app-page">
    <header className="dashboard-header"><div><Link to="/groups">← {t('내 그룹', 'My groups')}</Link><h1>{dashboard?.groupName ?? t('그룹', 'Group')} {t('대시보드', 'Dashboard')}</h1><p>{t(`${periodLabel}의 업무와 일정을 모아봤어요.`, `Tasks and events for ${periodLabel}.`)}</p></div><div className="group-dashboard-actions"><button className="primary create-action" type="button" onClick={() => setShowCreate(true)}><span aria-hidden="true">＋</span> {t('새 업무', 'New task')}</button><Link className="danger button-link dashboard-emergency-link" to={`/groups/${groupId}/emergency-issues`}>! {t('긴급 이슈', 'Urgent issue')}</Link><Link className="secondary dashboard-project-link" to={`/groups/${groupId}/projects`}>{t('프로젝트', 'Projects')}</Link><Link className="secondary dashboard-files-link" to={`/groups/${groupId}/files`}>{t('파일', 'Files')}</Link><Link className="settings-icon-button" to={`/groups/${groupId}`} aria-label={t('그룹 설정', 'Group settings')}>⚙</Link></div></header>
    ...
    {dashboard && <>
      {openEmergencyIssues.length>0&&<Link className="dashboard-emergency-banner" to={`/groups/${groupId}/emergency-issues`}><span>!</span><div><strong>{t(`대응이 필요한 긴급 이슈 ${openEmergencyIssues.length}건`,`Urgent issues requiring action: ${openEmergencyIssues.length}`)}</strong><small>{openEmergencyIssues.slice(0,2).map(issue=>`${issue.projectName} · ${issue.title}`).join(' / ')}</small></div><b>{t('확인하기','Review')} →</b></Link>}
      <div className="dashboard-analysis-heading"><div><span className="page-eyebrow">ANALYSIS</span><h2>{t('기간별 상세 현황', 'Period analysis')}</h2><p>{t('필요할 때 기간을 바꿔 업무 흐름과 리포트를 자세히 확인하세요.', 'Change the period when you need detailed workflow and reports.')}</p></div></div>
      ...
      <section className="collaboration-command-center"><header><div><span className="page-eyebrow">TEAM NOW</span><h2>{t('지금 우리 팀이 하는 일', 'What the team is working on')}</h2><p>{t('프로젝트와 주제, 담당 업무를 한눈에 확인하세요.', 'See projects, topics, and assigned work at a glance.')}</p></div><div><Link className="secondary button-link" to={`/groups/${groupId}/projects`}>{t('프로젝트 관리', 'Manage projects')}</Link><Link className="primary button-link" to={`/groups/${groupId}/chat`}>{t('팀 채팅', 'Team chat')}</Link></div></header>
        <div className="team-now-summary"><div><span>{t('진행 프로젝트', 'Active projects')}</span><strong>{projects.length}</strong></div><div><span>{t('진행 업무', 'Active tasks')}</span><strong>{activeTeamTasks.length}</strong></div><div className={dashboard.statuses.delayed > 0 ? 'attention' : ''}><span>{t('지연 업무', 'Overdue')}</span><strong>{dashboard.statuses.delayed}</strong></div><div className={unlinkedTasks.length > 0 ? 'attention' : ''}><span>{t('프로젝트 미연결', 'Not linked')}</span><strong>{unlinkedTasks.length}</strong></div></div>
        {projects.length === 0 ? <div className="project-onboarding"><span>1</span><div><strong>{t('먼저 프로젝트를 만들어 주세요', 'Create your first project')}</strong><p>{t('프로젝트를 만든 뒤 주제를 나누고, 각 주제에 업무를 연결하면 팀 현황이 자동으로 정리됩니다.', 'Create a project, add topics, and link tasks to build the team view automatically.')}</p></div><Link className="primary button-link" to={`/groups/${groupId}/projects`}>{t('프로젝트 만들기', 'Create project')}</Link></div> : <div className="live-project-grid">{projects.map((project) => <LiveProjectCard key={project.id} project={project} tasks={allTasks.filter((task) => task.projectId === project.id)} topics={topicsByProject[project.id] ?? []} language={language} />)}</div>}
        {unlinkedTasks.length > 0 && <div className="unlinked-task-notice"><span>!</span><div><strong>{t(`프로젝트에 연결되지 않은 진행 업무가 ${unlinkedTasks.length}개 있습니다.`, `${unlinkedTasks.length} active tasks are not linked to a project.`)}</strong><p>{t('업무 상세에서 프로젝트와 주제를 지정하면 팀 현황에 정확히 표시됩니다.', 'Assign a project and topic from task details for accurate team visibility.')}</p></div><Link to={`/groups/${groupId}/tasks`}>{t('업무 정리하기', 'Organize tasks')} →</Link></div>}
      </section>
      ...
      <section className="dashboard-panel team-workload-panel unified-member-work"><div className="dashboard-panel-title inline"><div><span className="page-eyebrow">TEAM WORKLOAD</span><h2>{t('팀원별 담당·진행 업무', 'Assigned and current work by member')}</h2><p>{t('전체 담당 현황과 지금 진행하는 프로젝트·주제·업무를 함께 보여줍니다.', 'See total workload together with each member’s current project, topic, and task.')}</p></div><span className="member-count">{t(`${dashboard.members.length}명`, `${dashboard.members.length} members`)}</span></div><div className="member-metrics">{dashboard.members.map((member) => { const profile = groupMembers.find((value) => value.id === member.memberId); const memberTasks = activeTeamTasks.filter((task) => task.assigneeMemberId === member.memberId).sort(currentTaskOrder); return <article key={member.memberId}><div className="member-metric-heading"><span className="member-metric-avatar" aria-hidden="true">{profile?.profileImageUrl ? <AuthenticatedImage src={profile.profileImageUrl} alt="" /> : member.nickname.slice(0, 1)}</span><span><strong>{member.nickname}</strong><small>{member.role === 'LEADER' ? t('팀장', 'Leader') : t('팀원', 'Member')}</small></span></div><dl>{(['assigned', 'active', 'completed', 'delayed'] as MemberMetricKey[]).map(metric => <div key={metric}><dt>{memberMetricLabel(metric, language)}</dt><dd><button type="button" onClick={() => setMemberTaskModal({ member, metric })} aria-label={t(`${member.nickname}님의 ${memberMetricLabel(metric, language)} 업무 보기`, `View ${member.nickname}'s ${memberMetricLabel(metric, language)} tasks`)}>{memberMetricCount(member, metric)}</button></dd></div>)}<div><dt>{t('기한 준수', 'On time')}</dt><dd>{rate(member.onTimeRatePercent)}</dd></div></dl><div className="member-current-tasks"><strong>{t('현재 진행 업무', 'Current work')}</strong>{memberTasks.length === 0 ? <p>{t('현재 담당 중인 업무가 없습니다.', 'No active assigned work.')}</p> : memberTasks.slice(0, 3).map((task) => <CurrentTaskLink task={task} key={task.id} language={language} />)}{memberTasks.length > 3 && <Link className="member-more-tasks" to={`/groups/${groupId}/tasks`}>{t(`${memberTasks.length - 3}개 더 보기`, `View ${memberTasks.length - 3} more`)}</Link>}</div></article>; })}</div></section>
...
function TaskLink({ task }: { task: DashboardTask }) { const { t, language } = useLanguage(); const value = taskStatusLabels[task.status]; return <Link to={`/tasks/${task.id}`}><div><strong>{task.title}</strong>{task.projectName && <span className="task-project-context">{task.projectName}{task.projectTopicTitle ? ` · ${task.projectTopicTitle}` : ''}</span>}{task.delayed && <span>{t('지연', 'Overdue')}</span>}</div><small>{value ? value[language === 'ko' ? 0 : 1] : task.status} · {task.assigneeNickname ?? t('담당자 미지정', 'Unassigned')} · {task.dueAt?.slice(0, 16) ?? t('마감 없음', 'No due date')}</small></Link>; }
function LiveProjectCard({ project, tasks, topics, language }: { project: ProjectResponse; tasks: TaskResponse[]; topics: ProjectIssue[]; language: 'ko' | 'en' }) {
  const active = tasks.filter((task) => !['COMPLETED', 'REJECTED', 'CANCELLED'].includes(task.status));
  const completed = tasks.filter((task) => task.status === 'COMPLETED').length;
  const progress = tasks.length === 0 ? 0 : Math.round(completed * 100 / tasks.length);
  const projectStatus = ({ PLANNED: ['준비', 'Planned'], ACTIVE: ['진행', 'Active'], ON_HOLD: ['보류', 'On hold'], COMPLETED: ['완료', 'Completed'], ARCHIVED: ['보관', 'Archived'] } as Record<ProjectResponse['status'], [string, string]>)[project.status][language === 'ko' ? 0 : 1];
  return <article><header><div><span className={`project-status status-${project.status.toLowerCase()}`}>{projectStatus}</span><h3><Link to={`/projects/${project.id}/flow`}>{project.name}</Link></h3></div><strong>{progress}%</strong></header><progress value={progress} max={100} /><div className="live-project-counts"><span>{language === 'ko' ? `진행 ${active.length}` : `${active.length} active`}</span><span>{language === 'ko' ? `완료 ${completed}` : `${completed} done`}</span><span>{language === 'ko' ? `주제 ${topics.length}` : `${topics.length} topics`}</span></div><div className="live-project-topics">{topics.length === 0 ? <p>{language === 'ko' ? '아직 주제가 없습니다.' : 'No topics yet.'}</p> : topics.slice(0, 4).map((topic) => { const topicTasks = tasks.filter((task) => task.projectTopicId === topic.id); return <Link to={`/projects/${project.id}/flow`} key={topic.id}><span>{topic.title}</span><b>{topicTasks.filter((task) => !['COMPLETED', 'REJECTED', 'CANCELLED'].includes(task.status)).length}/{topicTasks.length}</b></Link>; })}</div><footer><Link to={`/projects/${project.id}/flow`}>{language === 'ko' ? '프로젝트 현황 보기' : 'Open project'} →</Link><Link to={`/groups/${project.groupId}/tasks?projectId=${project.id}&create=1`}>＋ {language === 'ko' ? '업무 추가' : 'Add task'}</Link></footer></article>;
}
function CurrentTaskLink({ task, language }: { task: TaskResponse; language: 'ko' | 'en' }) {
  const status = taskStatusLabels[task.status]?.[language === 'ko' ? 0 : 1] ?? task.status;
  return <Link className="current-task-link" to={`/tasks/${task.id}`}><span className={`issue-status issue-${task.status.toLowerCase()}`}>{status}</span><div><strong>{task.title}</strong><small>{task.projectName ? `${task.projectName}${task.projectTopicTitle ? ` · ${task.projectTopicTitle}` : ''}` : language === 'ko' ? '프로젝트 미연결' : 'No project'}{task.dueAt ? ` · ${task.dueAt.slice(5, 10)}` : ''}</small></div></Link>;
}
function currentTaskOrder(a: TaskResponse, b: TaskResponse) { const rank: Record<TaskResponse['status'], number> = { ON_HOLD: 0, IN_PROGRESS: 1, TODO: 2, REQUESTED: 3, COMPLETED: 4, REJECTED: 5, CANCELLED: 6 }; return rank[a.status] - rank[b.status] || Number(b.delayed) - Number(a.delayed); }
