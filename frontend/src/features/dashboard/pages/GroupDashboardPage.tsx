...
export function GroupDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([groupApi.get(groupId), groupApi.members(groupId), projectApi.list(groupId)]).then(([value, members, projectValues]) => {
  ...
  return <><AppNavigation /><main className="group-dashboard-page app-page">
    <header className="dashboard-header"><div><Link to="/groups">← {t('내 그룹', 'My groups')}</Link><h1>{dashboard?.groupName ?? t('그룹', 'Group')} {t('대시보드', 'Dashboard')}</h1><p>{t(`${periodLabel}의 업무와 일정을 모아봤어요.`, `Tasks and events for ${periodLabel}.`)}</p></div><div className="group-dashboard-actions"><button className="primary create-action" type="button" onClick={() => setShowCreate(true)}><span aria-hidden="true">＋</span> {t('새 업무', 'New task')}</button><Link className="secondary dashboard-project-link" to={`/groups/${groupId}/projects`}>{t('프로젝트', 'Projects')}</Link><Link className="settings-icon-button" to={`/groups/${groupId}`} aria-label={t('그룹 설정', 'Group settings')}>⚙</Link></div></header>
    ...
    {dashboard && <>
      {projects.length > 0 && <section className="dashboard-project-overview"><header><div><span className="page-eyebrow">PROJECTS</span><h2>{t('프로젝트 진행 현황', 'Project progress')}</h2></div><Link to={`/groups/${groupId}/projects`}>{t('전체 프로젝트', 'All projects')} →</Link></header><div>{projects.slice(0, 6).map((project) => { const linked = dashboard.periodTasks.filter((task) => task.projectId === project.id); const completed = linked.filter((task) => task.status === 'COMPLETED').length; return <Link to={`/projects/${project.id}/flow`} key={project.id}><span className={`project-status status-${project.status.toLowerCase()}`}>{project.name}</span><strong>{linked.length === 0 ? '—' : `${Math.round(completed * 100 / linked.length)}%`}</strong><small>{t(`선택 기간 업무 ${linked.length}건 · 완료 ${completed}건`, `${linked.length} period tasks · ${completed} completed`)}</small></Link>; })}</div></section>}
      ...
      <section className="dashboard-panel team-workload-panel"><div className="dashboard-panel-title inline"><div><span className="page-eyebrow">TEAM WORKLOAD</span><h2>{t('팀원별 담당 현황', 'Workload by member')}</h2><p>{t('담당·진행·완료·지연 수치를 누르면 실제 업무 목록을 볼 수 있습니다.', 'Select Assigned, Active, Done, or Overdue to see the underlying tasks.')}</p></div><span className="member-count">{t(`${dashboard.members.length}명`, `${dashboard.members.length} members`)}</span></div><div className="member-metrics">{dashboard.members.map((member) => { const profile = groupMembers.find((value) => value.id === member.memberId); return <article key={member.memberId}><div className="member-metric-heading"><span className="member-metric-avatar" aria-hidden="true">{profile?.profileImageUrl ? <AuthenticatedImage src={profile.profileImageUrl} alt="" /> : member.nickname.slice(0, 1)}</span><span><strong>{member.nickname}</strong><small>{member.role === 'LEADER' ? t('팀장', 'Leader') : t('팀원', 'Member')}</small></span></div><dl>{(['assigned', 'active', 'completed', 'delayed'] as MemberMetricKey[]).map(metric => <div key={metric}><dt>{memberMetricLabel(metric, language)}</dt><dd><button type="button" onClick={() => setMemberTaskModal({ member, metric })} aria-label={t(`${member.nickname}님의 ${memberMetricLabel(metric, language)} 업무 보기`, `View ${member.nickname}'s ${memberMetricLabel(metric, language)} tasks`)}>{memberMetricCount(member, metric)}</button></dd></div>)}<div><dt>{t('기한 준수', 'On time')}</dt><dd>{rate(member.onTimeRatePercent)}</dd></div></dl></article>; })}</div></section>
