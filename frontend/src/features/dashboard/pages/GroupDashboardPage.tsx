...
export function GroupDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([groupApi.get(groupId), groupApi.members(groupId)]).then(([value, members]) => {
  ...
  return <><AppNavigation /><main className="group-dashboard-page app-page">
    <header className="dashboard-header"><div><Link to="/groups">← {t('내 그룹', 'My groups')}</Link><h1>{dashboard?.groupName ?? t('그룹', 'Group')} {t('대시보드', 'Dashboard')}</h1><p>{t(`${periodLabel}의 업무와 일정을 모아봤어요.`, `Tasks and events for ${periodLabel}.`)}</p></div><div className="group-dashboard-actions"><button className="primary create-action" type="button" onClick={() => setShowCreate(true)}><span aria-hidden="true">＋</span> {t('새 업무', 'New task')}</button><Link className="secondary dashboard-project-link" to={`/groups/${groupId}/projects`}>{t('프로젝트·이슈', 'Projects & issues')}</Link><Link className="secondary" to={`/groups/${groupId}/tasks`}>{t('업무', 'Tasks')}</Link><Link className="secondary" to={`/groups/${groupId}/chat`}>{t('채팅', 'Chat')}</Link><Link className="settings-icon-button" to={`/groups/${groupId}`} aria-label={t('그룹 설정', 'Group settings')}>⚙</Link></div></header>
    <nav className="dashboard-workspace-shortcuts" aria-label={t('그룹 주요 기능', 'Group workspace shortcuts')}><Link to={`/groups/${groupId}/projects`}><span aria-hidden="true">◇</span><span><strong>{t('프로젝트와 이슈', 'Projects and issues')}</strong><small>{t('프로젝트를 만들고 이슈 Flow를 관리합니다.', 'Create projects and manage issue flows.')}</small></span><b aria-hidden="true">→</b></Link><Link to={`/groups/${groupId}/tasks`}><span aria-hidden="true">✓</span><span><strong>{t('업무 관리', 'Task management')}</strong><small>{t('요청·승인·담당 업무를 확인합니다.', 'Review requests, approvals, and assignments.')}</small></span><b aria-hidden="true">→</b></Link><Link to={`/groups/${groupId}/chat`}><span aria-hidden="true">○</span><span><strong>{t('팀 채팅', 'Team chat')}</strong><small>{t('프로젝트별 채팅방에서 바로 대화합니다.', 'Talk directly in project channels.')}</small></span><b aria-hidden="true">→</b></Link></nav>
