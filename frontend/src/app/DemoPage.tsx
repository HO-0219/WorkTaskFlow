...
export function DemoPage() {
  ...
  return <div className="demo-page">
    <aside className="demo-nav">
      <Link className="demo-brand" to="/"><span><BrandMark /></span><strong>{t('퇴근', 'toegeun')}</strong></Link>
...
function DemoHome({ onOpenGroup, onOpenTasks, onOpenAlerts }: { onOpenGroup: () => void; onOpenTasks: () => void; onOpenAlerts: () => void }) {
  ...
  return <section className="demo-view">
    ...
    <div className="demo-home-grid">
      <article className="demo-panel demo-home-group"><header><div><span>SHORTCUTS</span><h2>{t('바로가기', 'Shortcuts')}</h2></div></header><button type="button" onClick={onOpenGroup}><span>퇴</span><div><strong>{t('퇴근 런칭 준비팀', 'toegeun Launch Team')}</strong><small>{t('팀 대시보드 열기', 'Open group dashboard')}</small></div><b>›</b></button></article>
      ...
      <article className="demo-panel demo-upcoming"><header><div><span>SCHEDULE</span><h2>{t('다가오는 일정', 'Upcoming events')}</h2></div></header><div><span><b>31</b><small>JUL</small></span><p><strong>{t('주간 진행 공유', 'Weekly progress sync')}</strong><small>10:00 · {t('퇴근 런칭 준비팀', 'toegeun Launch Team')}</small></p></div><div><span><b>01</b><small>AUG</small></span><p><strong>{t('출시 전 보안 점검', 'Pre-launch security review')}</strong><small>14:00 · Online</small></p></div></article>
...
function DemoMembers({ onBack, onOpenTasks }: { onBack: () => void; onOpenTasks: () => void }) {
  const { t } = useLanguage();
  return <section className="demo-view">
    <header className="demo-view-heading"><div><button className="demo-back-link" type="button" onClick={onBack}>← {t('그룹 대시보드', 'Group dashboard')}</button><span className="page-eyebrow">TEAM MEMBERS</span><h1>{t('팀원', 'Team members')}</h1><p>{t('퇴근 런칭 준비팀에서 함께하는 사람과 역할을 확인하세요.', 'See everyone working in the toegeun Launch Team and their roles.')}</p></div><div className="demo-heading-actions"><button type="button" onClick={onOpenTasks}>{t('업무 보기', 'View tasks')}</button><button type="button" disabled>{t('초대·설정', 'Invites & settings')}</button></div></header>
