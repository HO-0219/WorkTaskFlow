...
type DemoView = 'home' | 'groups' | 'dashboard' | 'tasks' | 'members' | 'assistant' | 'calendar' | 'notifications' | 'profile';
...
export function DemoPage() {
  ...
  return <div className="demo-page">
    <aside className="demo-nav">
      <Link className="demo-brand" to="/"><span><BrandMark /></span><strong>{t('Gearvia', 'Gearvia')}</strong></Link>
      ...
      <label className="demo-team-switcher"><span>{t('공간 이동', 'Switch workspace')}</span><select value={['dashboard', 'tasks', 'members', 'assistant'].includes(view) ? 'team' : ''} onChange={(event) => setView(event.target.value === 'team' ? 'dashboard' : 'groups')}><option value="">{t('전체 그룹 보기', 'View all groups')}</option><option value="team">◆ {t('Gearvia 런칭 준비팀', 'Gearvia Launch Team')}</option></select><small>{t('실제 서비스와 같은 방식으로 공간을 이동합니다.', 'Switch spaces just as you would in the live service.')}</small></label>
      <nav aria-label={t('데모 메뉴', 'Demo navigation')}>
        ...
        <DemoNavButton active={view === 'assistant'} icon="✦" onClick={() => setView('assistant')}>{t('AI 비서', 'Assistant')}</DemoNavButton>
      ...
      <div className="demo-profile"><span>김</span><div><strong>{t('김서준 팀장', 'Seo-jun Kim')}</strong><small>demo@gearvia.local</small></div></div>
    </aside>
...
function DemoHome({ onOpenGroup, onOpenTasks, onOpenAlerts }: { onOpenGroup: () => void; onOpenTasks: () => void; onOpenAlerts: () => void }) {
  ...
  return <section className="demo-view">
    ...
    <div className="demo-home-grid">
      <article className="demo-panel demo-home-group"><header><div><span>SHORTCUTS</span><h2>{t('바로가기', 'Shortcuts')}</h2></div></header><button type="button" onClick={onOpenGroup}><span>G</span><div><strong>{t('Gearvia 런칭 준비팀', 'Gearvia Launch Team')}</strong><small>{t('팀 대시보드 열기', 'Open group dashboard')}</small></div><b>›</b></button></article>
      ...
      <article className="demo-panel demo-upcoming"><header><div><span>SCHEDULE</span><h2>{t('다가오는 일정', 'Upcoming events')}</h2></div></header><div><span><b>31</b><small>JUL</small></span><p><strong>{t('주간 진행 공유', 'Weekly progress sync')}</strong><small>10:00 · {t('Gearvia 런칭 준비팀', 'Gearvia Launch Team')}</small></p></div><div><span><b>01</b><small>AUG</small></span><p><strong>{t('출시 전 보안 점검', 'Pre-launch security review')}</strong><small>14:00 · Online</small></p></div></article>
...
function DemoMembers({ onBack, onOpenTasks }: { onBack: () => void; onOpenTasks: () => void }) {
  const { t } = useLanguage();
  return <section className="demo-view">
    <header className="demo-view-heading"><div><button className="demo-back-link" type="button" onClick={onBack}>← {t('그룹 대시보드', 'Group dashboard')}</button><span className="page-eyebrow">TEAM MEMBERS</span><h1>{t('팀원', 'Team members')}</h1><p>{t('Gearvia 런칭 준비팀에서 함께하는 사람과 역할을 확인하세요.', 'See everyone working in the Gearvia Launch Team and their roles.')}</p></div><div className="demo-heading-actions"><button type="button" onClick={onOpenTasks}>{t('업무 보기', 'View tasks')}</button><button type="button" disabled>{t('초대·설정', 'Invites & settings')}</button></div></header>
...
function DemoDashboard({ onOpenTasks, onOpenMembers, onSelect }: { onOpenTasks: () => void; onOpenMembers: () => void; onSelect: (task: DemoTask) => void }) {
  ...
  return <section className="demo-view">
    ...
    <article className="demo-panel demo-report-panel"><header><div><span>AI REPORTS</span><h2>{t('AI 업무 리포트', 'AI work report')}</h2><p>{t('저장된 업무 기록에서 핵심 변화, 위험과 다음 행동을 찾아 주간 리포트로 정리합니다.', 'Turn recorded work into a weekly brief of key changes, risks, and next actions.')}</p></div><b>{t('멤버십 활성', 'Membership active')}</b></header><div><label><span>{t('범위', 'Scope')}</span><select defaultValue="GROUP"><option value="GROUP">{t('그룹 전체', 'Whole group')}</option></select></label><label><span>{t('기간', 'Period')}</span><select defaultValue="WEEKLY"><option value="WEEKLY">{t('7월 4주차', 'July week 4')}</option></select></label><button className="demo-ai-report-button" type="button" onClick={() => setReportOpen((value) => !value)}>{reportOpen ? t('미리보기 닫기', 'Close preview') : t('AI 리포트 보기', 'View AI report')}</button></div>{reportOpen && <div className="demo-ai-report"><header><div><span>{t('주간 실행 브리프', 'WEEKLY EXECUTIVE BRIEF')}</span><h3>{t('출시 준비는 계획대로 진행 중이지만, 결제 검증 업무가 일정 위험을 만들고 있습니다.', 'Launch preparation is on track, but payment verification is creating schedule risk.')}</h3></div><b>R3</b></header><div className="demo-ai-report-kpis"><p><small>{t('완료율', 'Completion')}</small><strong>76%</strong><i>+5%p</i></p><p><small>{t('완료 업무', 'Completed')}</small><strong>19</strong><i>+6</i></p><p><small>{t('지연·위험', 'Delayed / at risk')}</small><strong>3</strong><i className="risk">{t('확인 필요', 'Review')}</i></p></div><div className="demo-ai-report-grid"><section><b>{t('핵심 위험', 'KEY RISK')}</b><h4>{t('결제 검증 결과가 배포 결정의 선행 조건입니다.', 'Payment verification is blocking the release decision.')}</h4><p>{t('마스킹 검증이 오늘 마감이며 체크리스트 1개가 남았습니다. 담당자 진행률은 75%입니다.', 'Masking verification is due today with one checklist item remaining. Owner progress is 75%.')}</p><small>#2 {t('결제 정보 마스킹 검증', 'Payment data masking review')}</small></section><section><b>{t('다음 행동', 'NEXT ACTION')}</b><h4>{t('오후 배포 전 검증 결과를 먼저 확인하세요.', 'Review verification before this afternoon’s release.')}</h4><p>{t('정품질 담당자의 최종 결과 공유 후 운영 배포 체크리스트를 승인하는 순서가 안전합니다.', 'Wait for the QA owner’s final result before approving the production checklist.')}</p><small>{t('담당 김서준 · 오늘 15:00', 'Owner Seo-jun Kim · Today 15:00')}</small></section></div></div>}<small>ⓘ {t('데모에서는 샘플 분석만 보여주며 실제 AI 생성·다운로드·메일 발송은 실행하지 않습니다.', 'The demo shows a sample analysis; AI generation, downloads, and email delivery do not run.')}</small></article>
  </section>;
}
...
function DemoAssistant() {
  const { t } = useLanguage();
  const prompts = [
    [t('마감 임박 정리', 'Due-soon summary'), t('이번 주 마감 임박 업무를 정리해줘.', 'Summarize work due this week.')],
    [t('배포 업무 만들기', 'Create release task'), t('배포 점검 업무와 체크리스트를 만들어줘.', 'Create a release-check task and checklist.')],
    [t('막힌 지점 찾기', 'Find blockers'), t('진행 중인 업무의 막힌 지점을 알려줘.', 'Find blockers in active work.')],
  ];
  const [selected, setSelected] = useState(0);
  return <section className="demo-view demo-assistant-view"><header className="demo-view-heading"><div><span className="page-eyebrow">AI ASSISTANT</span><h1>{t('업무 비서', 'Work assistant')}</h1><p>{t('실제 팀 데이터와 권한을 바탕으로 답하고, 변경 작업은 확인 후 실행하는 흐름을 체험하세요.', 'Preview how the assistant answers from team data and confirms every change before execution.')}</p></div><span className="demo-membership-live"><i />{t('멤버십 활성', 'Membership active')}</span></header><div className="demo-assistant-layout"><article className="demo-assistant-chat"><header><div><span>✦</span><p><strong>{t('Gearvia AI', 'Gearvia AI')}</strong><small>{t('Gearvia 런칭 준비팀의 업무를 확인 중', 'Reading work in Gearvia Launch Team')}</small></p></div><b>{t('온라인', 'Online')}</b></header><div className="demo-assistant-prompts">{prompts.map(([label], index) => <button className={selected === index ? 'active' : ''} type="button" onClick={() => setSelected(index)} key={label}>{label}</button>)}</div><div className="demo-assistant-messages"><p className="user">{prompts[selected][1]}</p>{selected === 0 ? <><div className="ai"><i>AI</i><p>{t('이번 주 마감 임박 업무는 3건입니다. 그중 결제 정보 마스킹 검증은 오늘 마감이고 체크리스트 1개가 남아 가장 먼저 확인해야 합니다.', 'Three tasks are due this week. Payment data masking review is due today with one checklist item left, so it needs attention first.')}</p></div><div className="demo-assistant-result"><b>{t('우선 확인', 'REVIEW FIRST')}</b><strong>{t('결제 정보 마스킹 검증', 'Payment data masking review')}</strong><span>{t('정품질 · 오늘 17:00 · 진행률 75%', 'QA Owner · Today 17:00 · 75% complete')}</span></div></> : selected === 1 ? <><div className="ai"><i>AI</i><p>{t('운영 배포 점검 업무를 아래 내용으로 만들 수 있어요. 실행 전에 내용을 확인해 주세요.', 'I can create the production release-check task below. Review it before execution.')}</p></div><div className="demo-assistant-action"><b>{t('실행 전 확인', 'REVIEW BEFORE ACTION')}</b><strong>{t('운영 배포 점검', 'Production release check')}</strong><span>{t('긴급 · 오늘 18:00 · 체크리스트 3개', 'Urgent · Today 18:00 · 3 checklist items')}</span><button type="button" disabled>{t('확인하고 실행', 'Confirm and run')}</button></div></> : <><div className="ai"><i>AI</i><p>{t('진행 중 업무 8건 중 2건에 막힌 지점이 있습니다. 결제 검증은 최종 로그 확인, 약관 검토는 법무 회신을 기다리고 있어요.', 'Two of eight active tasks have blockers: payment verification awaits the final log check, and terms review awaits legal feedback.')}</p></div><div className="demo-assistant-result warning"><b>{t('병목 2건', '2 BLOCKERS')}</b><strong>{t('외부 회신 1 · 내부 확인 1', '1 external response · 1 internal check')}</strong><span>{t('근거 업무를 열어 담당자와 최근 댓글을 확인할 수 있습니다.', 'Open source tasks to review owners and recent comments.')}</span></div></>}</div><footer><textarea readOnly value={prompts[selected][1]} /><button type="button" disabled>{t('보내기', 'Send')}</button></footer></article><aside className="demo-assistant-safety"><span>✓</span><h2>{t('AI가 임의로 바꾸지 않아요.', 'AI never changes work on its own.')}</h2><p>{t('선택한 그룹의 데이터만 읽고, 내 권한 범위의 작업만 제안하며, 실행 전에는 항상 요약을 보여줍니다.', 'It reads only the selected group, proposes only permitted actions, and always previews them before execution.')}</p><ol><li><b>01</b>{t('그룹 맥락 확인', 'Read group context')}</li><li><b>02</b>{t('계정 권한 검사', 'Check account permissions')}</li><li><b>03</b>{t('사용자 확인 후 실행', 'Run after confirmation')}</li></ol><small>{t('읽기 전용 데모이므로 실제 업무는 변경되지 않습니다.', 'This read-only demo never changes real work.')}</small></aside></div></section>;
}
