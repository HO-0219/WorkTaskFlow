...
function PublicHeader() {
  ...
  return <header className="public-header"><Link to="/" className="landing-brand"><BrandMark /><strong>{t('퇴근', 'toegeun')}</strong></Link><nav>
...
export function ProductPage() {
  const { t } = useLanguage();
  return <PublicLayout eyebrow="PRODUCT" title={t('팀의 업무 흐름을 한곳에서.', 'One place for your team workflow.')} intro={t('퇴근은 요청·승인·담당·실행·보고를 연결하고, AI 비서가 현재 권한 안에서 확인된 작업을 실행하는 업무 협업 서비스입니다.', 'toegeun connects requests, approvals, ownership, execution, and reporting, with an AI assistant that runs confirmed actions within your permissions.')}>
    <div className="product-catalog">
      <article className="featured"><span>{t('현재 이용 가능', 'Available now')}</span><h2>{t('퇴근', 'toegeun')}</h2><p>{t('요청·승인·담당자 선택·진행·완료와 팀 일정을 한 흐름으로 관리하는 업무 협업 서비스입니다.', 'A work collaboration service connecting requests, approvals, ownership, progress, completion, and team schedules.')}</p><div><small>{t('업무 관리', 'Work management')}</small><small>{t('팀 협업', 'Team collaboration')}</small><small>PWA</small></div><Link to="/login">{t('무료로 시작', 'Get started')} →</Link></article>
    ...
    <h2 className="public-section-title">{t('퇴근이 제공하는 것', 'What toegeun offers')}</h2>
    <InfoGrid cards={[
...
export function ContactPage() {
  ...
  return <PublicLayout eyebrow="CONTACT" title={t('궁금한 점을 편하게 남겨 주세요.', 'Let’s talk.')} intro={t('제품 사용, 기능 제안과 향후 B2B 구축에 대한 문의를 이메일로 받고 있습니다.', 'We welcome questions about the product, feature ideas, and future B2B deployment by email.')}>
    <InfoGrid cards={[
      [t('제품·기능', 'Product & features'), t('퇴근 사용 방법, 불편한 점과 필요한 기능을 알려 주세요.', 'Ask about toegeun, report friction, or suggest a feature.')],
...
export function PrivacyPage() {
  const { t } = useLanguage();
  return <PublicLayout policy eyebrow="PRIVACY" title={t('개인정보 처리방침', 'Privacy policy')} intro={t('퇴근은 서비스 제공에 필요한 최소한의 개인정보를 처리하고, 사용자가 자신의 정보를 통제할 수 있도록 합니다.', 'toegeun processes only the personal information needed to provide the service and helps users stay in control of it.')}>
    ...
    <PolicySection title={t('4. 외부 서비스와 처리 위탁', '4. External services')}><p>{t('Google OAuth 2.0은 사용자가 선택한 Google 로그인을 처리합니다. 퇴근은 인증 결과로 필요한 최소 프로필만 받아 계정을 생성하며, Google 계정 연결은 회원 탈퇴 시 삭제합니다. 이메일 발송과 인프라 운영을 위해 설정된 제공자를 이용할 수 있습니다. PWA 푸시를 허용하면 암호화된 알림이 Apple·Google·Mozilla·Microsoft 등 해당 브라우저의 푸시 제공자를 거쳐 전달됩니다. 결제수단을 등록하는 경우 카드 정보는 토스페이먼츠 화면에서 직접 처리되며 서비스 서버에는 카드번호·CVC가 저장되지 않습니다. 실제 운영 제공자, 처리 위치와 국외 이전 사항은 운영 전 최종 방침에 명시해야 합니다.', 'Google OAuth 2.0 handles Google sign-in selected by the user. toegeun receives only the minimum profile needed to create an account and deletes the Google account link when the account is deleted. Configured providers may support email and infrastructure. If you allow PWA push, encrypted alerts are routed through your browser provider, such as Apple, Google, Mozilla, or Microsoft. Card details are entered directly with Toss Payments and card number/CVC are not stored by toegeun. Final production vendors, processing locations, and international transfers must be listed before launch.')}</p></PolicySection>
...
export function TermsPage() {
  const { t } = useLanguage();
  return <PublicLayout policy eyebrow="TERMS" title={t('서비스 이용약관', 'Terms of service')} intro={t('퇴근을 안전하고 공정하게 이용하기 위한 기본 조건입니다.', 'These are the basic conditions for using toegeun safely and fairly.')}>
    <PolicySection title={t('1. 서비스의 목적', '1. Purpose')}><p>{t('퇴근은 그룹 업무, 일정, 알림과 협업 기록을 관리하는 서비스를 제공합니다.', 'toegeun provides group task, schedule, notification, and collaboration-record features.')}</p></PolicySection>
...
export function SiteMapPage() {
  ...
  return <PublicLayout eyebrow="SITE MAP" title={t('전체 사이트맵', 'Site map')} intro={t('퇴근의 주요 화면과 정보 구조를 한눈에 확인하세요.', 'Explore the main screens and information structure of toegeun.')}>
