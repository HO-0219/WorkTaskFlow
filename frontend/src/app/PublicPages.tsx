...
function PublicHeader() {
  ...
  return <header className="public-header"><Link to="/" className="landing-brand"><BrandMark /><strong>ToTaskFlow</strong></Link><nav>
...
export function ProductPage() {
  const { t } = useLanguage();
  return <PublicLayout eyebrow="PRODUCTS" title={t('우리가 만드는 제품을 한곳에서.', 'Everything we build, in one place.')} intro={t('현재는 ToTaskFlow에 집중하고 있습니다. 앞으로 업무 에이전트, 추가 기능과 이전 프로젝트도 제품 기준에 맞춰 이곳에 순차적으로 공개합니다.', 'We are focused on ToTaskFlow today. Work agents, extensions, and selected past projects will be added here as they become ready.')}>
    <div className="product-catalog">
      <article className="featured"><span>{t('현재 이용 가능', 'Available now')}</span><h2>ToTaskFlow</h2><p>{t('요청·승인·담당자 선택·진행·완료와 팀 일정을 한 흐름으로 관리하는 업무 협업 서비스입니다.', 'A work collaboration service connecting requests, approvals, ownership, progress, completion, and team schedules.')}</p><div><small>{t('업무 관리', 'Work management')}</small><small>{t('팀 협업', 'Team collaboration')}</small><small>PWA</small></div><Link to="/login">{t('무료로 시작', 'Get started')} →</Link></article>
    ...
    <h2 className="public-section-title">{t('ToTaskFlow가 제공하는 것', 'What ToTaskFlow offers')}</h2>
    <InfoGrid cards={[
...
export function ContactPage() {
  ...
  return <PublicLayout eyebrow="CONTACT" title={t('궁금한 점을 편하게 남겨 주세요.', 'Let’s talk.')} intro={t('제품 사용, 기능 제안과 향후 B2B 구축에 대한 문의를 이메일로 받고 있습니다.', 'We welcome questions about the product, feature ideas, and future B2B deployment by email.')}>
    <InfoGrid cards={[
      [t('제품·기능', 'Product & features'), t('ToTaskFlow 사용 방법, 불편한 점과 필요한 기능을 알려 주세요.', 'Ask about ToTaskFlow, report friction, or suggest a feature.')],
...
export function PrivacyPage() {
  const { t } = useLanguage();
  return <PublicLayout policy eyebrow="PRIVACY" title={t('개인정보 처리방침', 'Privacy policy')} intro={t('ToTaskFlow는 서비스 제공에 필요한 최소한의 개인정보를 처리하고, 사용자가 자신의 정보를 통제할 수 있도록 합니다.', 'ToTaskFlow processes only the personal information needed to provide the service and helps users stay in control of it.')}>
    ...
    <PolicySection title={t('4. 외부 서비스와 처리 위탁', '4. External services')}><p>{t('Google OAuth 2.0은 사용자가 선택한 Google 로그인을 처리합니다. ToTaskFlow는 인증 결과로 필요한 최소 프로필만 받아 계정을 생성하며, Google 계정 연결은 회원 탈퇴 시 삭제합니다. 이메일 발송과 인프라 운영을 위해 설정된 제공자를 이용할 수 있습니다. 결제수단을 등록하는 경우 카드 정보는 토스페이먼츠 화면에서 직접 처리되며 ToTaskFlow 서버에는 카드번호·CVC가 저장되지 않습니다. 실제 운영 제공자, 처리 위치와 국외 이전 사항은 운영 전 최종 방침에 명시해야 합니다.', 'Google OAuth 2.0 handles Google sign-in selected by the user. ToTaskFlow receives only the minimum profile needed to create an account and deletes the Google account link when the account is deleted. Configured providers may support email and infrastructure. Card details are entered directly with Toss Payments and card number/CVC are not stored by ToTaskFlow. Final production vendors, processing locations, and international transfers must be listed before launch.')}</p></PolicySection>
...
export function TermsPage() {
  const { t } = useLanguage();
  return <PublicLayout policy eyebrow="TERMS" title={t('서비스 이용약관', 'Terms of service')} intro={t('ToTaskFlow를 안전하고 공정하게 이용하기 위한 기본 조건입니다.', 'These are the basic conditions for using ToTaskFlow safely and fairly.')}>
