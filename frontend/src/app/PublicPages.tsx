...
function PublicHeader() {
  ...
  return <header className="public-header"><Link to="/" className="landing-brand"><BrandMark /><strong>{t('Gearvia', 'Gearvia')}</strong></Link><nav>
...
export function ProductPage() {
  const { t } = useLanguage();
  return <PublicLayout eyebrow="PRODUCT" title={t('팀의 업무 흐름을 한곳에서.', 'One place for your team workflow.')} intro={t('Gearvia는 요청·승인·담당·실행·보고를 연결하고, AI 비서가 현재 권한 안에서 확인된 작업을 실행하는 업무 협업 서비스입니다.', 'Gearvia connects requests, approvals, ownership, execution, and reporting, with an AI assistant that runs confirmed actions within your permissions.')}>
    <div className="product-catalog">
      <article className="featured"><span>{t('현재 이용 가능', 'Available now')}</span><h2>{t('Gearvia', 'Gearvia')}</h2><p>{t('요청·승인·담당자 선택·진행·완료와 팀 일정을 한 흐름으로 관리하는 업무 협업 서비스입니다.', 'A work collaboration service connecting requests, approvals, ownership, progress, completion, and team schedules.')}</p><div><small>{t('업무 관리', 'Work management')}</small><small>{t('팀 협업', 'Team collaboration')}</small><small>PWA</small></div><Link to="/login">{t('무료로 시작', 'Get started')} →</Link></article>
      <article><span>{t('현재 이용 가능', 'Available now')}</span><h2>{t('AI 업무 비서', 'AI work assistant')}</h2><p>{t('대화로 업무·체크리스트·댓글·멘션·알림을 제안하고, 권한 확인과 사용자 승인을 거쳐 실행합니다.', 'Use chat to propose tasks, checklists, comments, mentions, and notifications, then run them after permission checks and user confirmation.')}</p><small>{t('그룹별 대화 기록 지원', 'Workspace conversation history included')}</small></article>
    ...
    <h2 className="public-section-title">{t('Gearvia가 제공하는 것', 'What Gearvia offers')}</h2>
    <InfoGrid cards={[
...
export function PricingPage() {
  ...
  return <PublicLayout eyebrow="PRICING" title={t('필요한 기능으로 바로 시작하세요.', 'Start with the features you need.')} intro={t('핵심 협업 기능은 무료로 제공하며, 유료 그룹 기능은 결제 운영 승인 후 별도로 안내합니다.', 'Core collaboration features are free. Paid group features will be announced separately after payment operations are approved.')}>
    <div className="pricing-grid"><article><span>FREE</span><h2>{t('무료', 'Free')}</h2><strong>₩0</strong><p>{t('업무 흐름, 댓글, 알림, 캘린더와 한국어·영어 기본 리포트를 제공합니다.', 'Task workflows, comments, alerts, calendar, and core Korean/English reports are included.')}</p><Link to="/login">{t('무료로 시작', 'Get started')}</Link></article><article><span>PAID</span><h2>{t('유료 그룹', 'Paid group')}</h2><strong>{t('월 9,900원', '₩9,900/mo')}</strong><p>{t('AI 업무 비서와 AI 분석, 주간·월간 메일 리포트를 제공합니다. 멤버십은 등록한 카드로 매월 자동 갱신됩니다.', 'Includes the AI work assistant, AI analysis, and scheduled reports. Membership renews monthly on the registered card.')}</p><Link to="/paid-terms">{t('유료서비스 기준 보기', 'View paid terms')}</Link></article><article><span>B2B</span><h2>{t('팀 도입', 'Team rollout')}</h2><strong>{t('문의', 'Contact')}</strong><p>{t('인원·보안·운영 지원 범위를 확인한 뒤 도입 방식을 협의합니다.', 'We scope rollout options based on team size, security, and support needs.')}</p><Link to="/contact">{t('도입 문의', 'Contact sales')}</Link></article></div>
...
export function ContactPage() {
  ...
  return <PublicLayout eyebrow="CONTACT" title={t('궁금한 점을 편하게 남겨 주세요.', 'Let’s talk.')} intro={t('제품 사용, 기능 제안과 향후 B2B 구축에 대한 문의를 이메일로 받고 있습니다.', 'We welcome questions about the product, feature ideas, and future B2B deployment by email.')}>
    <InfoGrid cards={[
      [t('제품·기능', 'Product & features'), t('Gearvia 사용 방법, 불편한 점과 필요한 기능을 알려 주세요.', 'Ask about Gearvia, report friction, or suggest a feature.')],
...
export function PrivacyPage() {
  const { t } = useLanguage();
  return <PublicLayout policy eyebrow="PRIVACY" title={t('개인정보 처리방침', 'Privacy policy')} intro={t('Gearvia는 서비스 제공에 필요한 최소한의 개인정보를 처리하고, 사용자가 자신의 정보를 통제할 수 있도록 합니다.', 'Gearvia processes only the personal information needed to provide the service and helps users stay in control of it.')}>
    <PolicySection title={t('1. 처리하는 개인정보', '1. Information we process')}><p>{t('일반 회원가입 시 이름, 아이디, 이메일과 비밀번호의 단방향 해시를 처리합니다. Google 가입 시에는 Google 계정 식별자, 확인된 이메일과 이름을 처리하며 Google 비밀번호 또는 Google 액세스·리프레시 토큰은 저장하지 않습니다. 선택적으로 전화번호, 프로필 이미지, 업무 알림 및 마케팅 수신 동의를 처리합니다. PWA 푸시 알림을 허용하면 브라우저가 발급한 기기별 푸시 주소와 공개 암호화 키를 저장합니다. 서비스 이용 과정에서 접속 기록, 보안 감사 기록, 그룹·업무·댓글·캘린더 데이터가 생성될 수 있습니다.', 'For regular signup we process your name, username, email, and a one-way password hash. For Google signup we process the Google account identifier, verified email, and name; we do not store your Google password or Google access/refresh tokens. Phone number, profile image, work-notification preference, and marketing preference are optional. If you allow PWA push alerts, we store the device-specific push address and public encryption keys issued by your browser. Security logs and workspace content can be created as you use the service.')}</p></PolicySection>
    ...
    <PolicySection title={t('3. 보유 기간', '3. Retention')}><p>{t('계정 정보와 저장된 푸시 주소는 회원 탈퇴 시 익명화하거나 삭제하며, 브라우저 제공자가 만료되었다고 응답한 푸시 주소도 삭제합니다. 법령상 보존 의무 또는 분쟁 대응이 필요한 정보는 해당 목적과 기간 동안 분리 보관할 수 있습니다. 업무 상태 이력처럼 그룹의 협업 기록에 포함되는 정보는 서비스 기록 보존 정책에 따라 보존될 수 있습니다.', 'Account information and stored push addresses are deleted or anonymized on withdrawal; push addresses reported as expired by the browser provider are also deleted. Information required by law or for dispute handling may be retained separately for the applicable period. Collaboration records such as task history may remain under the service record-retention policy.')}</p></PolicySection>
    <PolicySection title={t('4. 외부 서비스와 처리 위탁', '4. External services')}><p>{t('Google OAuth 2.0은 사용자가 선택한 Google 로그인을 처리합니다. Gearvia는 인증 결과로 필요한 최소 프로필만 받아 계정을 생성하며, Google 계정 연결은 회원 탈퇴 시 삭제합니다. 이메일 발송과 인프라 운영을 위해 설정된 제공자를 이용할 수 있습니다. PWA 푸시를 허용하면 암호화된 알림이 Apple·Google·Mozilla·Microsoft 등 해당 브라우저의 푸시 제공자를 거쳐 전달됩니다. 결제수단을 등록하는 경우 카드 정보는 토스페이먼츠 화면에서 직접 처리되며 서비스 서버에는 카드번호·CVC가 저장되지 않습니다. 실제 운영 제공자, 처리 위치와 국외 이전 사항은 운영 전 최종 방침에 명시해야 합니다.', 'Google OAuth 2.0 handles Google sign-in selected by the user. Gearvia receives only the minimum profile needed to create an account and deletes the Google account link when the account is deleted. Configured providers may support email and infrastructure. If you allow PWA push, encrypted alerts are routed through your browser provider, such as Apple, Google, Mozilla, or Microsoft. Card details are entered directly with Toss Payments and card number/CVC are not stored by Gearvia. Final production vendors, processing locations, and international transfers must be listed before launch.')}</p></PolicySection>
...
export function TermsPage() {
  const { t } = useLanguage();
  return <PublicLayout policy eyebrow="TERMS" title={t('서비스 이용약관', 'Terms of service')} intro={t('Gearvia를 안전하고 공정하게 이용하기 위한 기본 조건입니다.', 'These are the basic conditions for using Gearvia safely and fairly.')}>
    <PolicySection title={t('1. 서비스의 목적', '1. Purpose')}><p>{t('Gearvia는 그룹 업무, 일정, 알림과 협업 기록을 관리하는 서비스를 제공합니다.', 'Gearvia provides group task, schedule, notification, and collaboration-record features.')}</p></PolicySection>
...
export function SiteMapPage() {
  ...
  return <PublicLayout eyebrow="SITE MAP" title={t('전체 사이트맵', 'Site map')} intro={t('Gearvia의 주요 화면과 정보 구조를 한눈에 확인하세요.', 'Explore the main screens and information structure of Gearvia.')}>
