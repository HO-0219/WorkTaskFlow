...
export function LandingPage() {
  ...
  return <main className="landing-page">
    <header className="landing-nav">
      <Link to="/" className="landing-brand"><BrandMark /><strong>{t('퇴근', 'toegeun')}</strong></Link>
    ...
    <section className="landing-hero">
      ...
      <button className="landing-pwa-hero" type="button" onClick={() => setInstallOpen(true)} disabled={installed}><span aria-hidden="true">↓</span>{installed ? t('퇴근 앱으로 실행 중', 'Running as the toegeun app') : t('앱스토어 없이, 이 기기에 퇴근 추가', 'Add toegeun to this device—no app store')}</button>
    ...
    <section className="landing-product" aria-label={t('제품 화면 미리보기', 'Product preview')}>
      ...
      <div className="landing-product-body">
        <aside><div className="mock-brand"><BrandMark />{t('퇴근', 'toegeun')}</div><span className="active">⌂ {t('홈', 'Home')}</span><span>♧ {t('그룹', 'Groups')}</span><span>□ {t('캘린더', 'Calendar')}</span><span>♢ {t('알림', 'Alerts')}</span><b>{t('로컬 알파 시연팀', 'Alpha demo team')}</b></aside>
    ...
    <section className="landing-name-story" aria-label={t('퇴근 브랜드 이야기', 'The toegeun name')}>
      <span>toegeun / 퇴근</span>
      <h2>{t('업무는 분명하게 남기고,', 'Keep work clear and accountable.')}<br /><em>{t('제시간에 퇴근하세요.', 'Leave work on time.')}</em></h2>
    ...
    <section className="landing-proof" aria-label={t('퇴근 핵심 가치', 'toegeun key value')}>
